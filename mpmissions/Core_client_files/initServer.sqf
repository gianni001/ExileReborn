...
switch (toLower worldName) do
{
    case "altis":
    {
        ...
        /*
        // To do -- create markers serverside script, remove from mission.sqm
        // [markerName (unique string),"markerType","markertext",[position],size A,size B]
        _markers =
        [
            // Boat
            ["ExileMarker%1","ExileBoatTraderIcon","Boat Trader",[13391.2,14547.2,0],0,0],
            ["ExileMarker%1","ExileNonConstructionZone","",[13391.2,14547.2,0],600,600]
            // Office
        ...
        {
            _mk = createMarker [format [(_x select 0),diag_tickTime],(_x select 3)];
            _mk setMarkerType (_x select 1);
            _mk setMarkerText (_x select 2);
            _mk setMarkerSize [(_x select 4),(_x select 5)];
        } forEach _markers;
        */
